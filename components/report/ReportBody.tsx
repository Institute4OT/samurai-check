...
const DEFAULT_LABELS: Record<keyof NormalizedCategoryScores, string> = {
  ...
  harassmentRisk: "無自覚ハラスメント傾向", // ← これを追加
};
...
export default function ReportBody({
  ...
  const gifts = (
    personal?.gifts?.length
      ? personal.gifts
      : top2.map(
          ([k]) =>
            `「${labels[k]}」が高く、チームを前に進める強みとして機能しています。`,
        )
  ).slice(0, 2);
  const challenges = (
    personal?.challenges?.length
      ? personal.challenges
      : bottom2.map(
          ([k]) =>
            `「${labels[k]}」は伸びしろ領域。次の90日で小さく改善を積み上げましょう。`,
        )
  ).slice(0, 2);
        ...
        <Card className="border rounded-2xl">
          <CardContent className="p-6 flex items-center gap-6">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-semibold">
                組織の成長に役立つ最新情報はオープンチャットで！
              </h3>
              {openChat?.linkHref && (
                <a
                  className="underline text-sm"
                  href={openChat.linkHref}
                  target="_blank"
