...
import ReportBody, { TypeContent, PersonalComments, OpenChatInfo } from './ReportBody';
...
export default function ReportTemplate({
  ...
  const radarData = useMemo(
    () => (Object.keys(normalizedScores) as (keyof NormalizedCategoryScores)[]).map((k) => ({
      category: LABELS[k],
    ...
    <div className="space-y-6">
      ...
      <header className="flex items-center gap-4">
        <div className="relative w-14 h-14">
          <Image src={kabuto.src} alt={`${kabuto.surname} kabuto`} fill sizes="56px" className="object-contain" />
        </div>
        <div className="flex-1">
          <h1 className="text-2xl font-bold">{title ?? 'AI時代の経営者｜武将タイプ診断レポート'}</h1>
          <p className="text-sm text-muted-foreground">
            {subtitle ?? 'AI時代に通用する“経営進化スピード”を診断'}／
            <span className="ml-1">ID: <span className="font-mono">{diagId}</span></span>
          </p>
        </div>
      </header>
      ...
      <Card className="border rounded-2xl">
        <CardContent className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">※各カテゴリは0〜3で正規化表示</p>
          </div>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radarData}>
                ...
                <Radar name="score" dataKey="value" strokeOpacity={0.9} fillOpacity={0.2} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      {/* CTA（会社規模別） */}
      <ReportCTA companySize={companySize} shareUrl={shareUrl} consultUrl={consultUrl} />
        ...
        <div className="flex items-center gap-3">
          {brandLogoSrc && (
            <div className="relative w-16 h-16">
              <Image src={brandLogoSrc} alt="IOT logo" fill sizes="64px" className="object-contain" />
            </div>
          )}
          <div className="text-sm">
            <a href={brandSiteUrl} target="_blank" rel="noopener noreferrer" className="underline">
