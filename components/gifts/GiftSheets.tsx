      ...
      <CardContent className="p-6 space-y-4">
        <header className="flex flex-col gap-1">
          ...
          <p className="text-xs text-muted-foreground">
            {data.company ?? '（会社名）'} / ResultID: {data.resultId ?? '-'} / Owner: {data.owner ?? '-'}
          </p>
        </header>
        ...
        <section>
          ...
          <ul className="list-disc pl-5 space-y-1">
            {data.strengths.map((s, i) => (
              <li key={i} className="text-sm leading-6">{s}</li>
            ))}
          </ul>
        </section>
        ...
        <section>
          ...
          <div className="rounded-lg border p-3">
            <p className="text-sm"><span className="font-medium">テーマ：</span>{data.focusIssue.label}</p>
            <p className="text-sm"><span className="font-medium">なぜ今：</span>{data.focusIssue.why}</p>
            {data.focusIssue.risks && (
              <p className="text-sm"><span className="font-medium">リスク/前提：</span>{data.focusIssue.risks}</p>
            )}
            {data.focusIssue.killCriteria && (
              <p className="text-sm"><span className="font-medium">中止/継続の判断基準：</span>{data.focusIssue.killCriteria}</p>
            )}
            {data.focusIssue.notThisQuarter && (
              <p className="text-sm"><span className="font-medium">今期はやらない：</span>{data.focusIssue.notThisQuarter}</p>
            )}
          </div>
        </section>
        ...
        {data.notes?.length ? (
          <section>
            ...
            <ul className="list-disc pl-5 space-y-1">
              {data.notes.map((n, i) => (
                <li key={i} className="text-sm leading-6">{n}</li>
              ))}
            </ul>
          </section>
        ) : null}
    ...
    <Card className="rounded-2xl border shadow-sm">
      <CardContent className="p-6 space-y-4">
        <header className="flex flex-col gap-1">
          ...
          <p className="text-xs text-muted-foreground">
            {data.company ?? '（会社名）'} / 期間：{data.horizon}（レビュー：{data.reviewCadence ?? '—'}）
          </p>
        </header>
        ...
        <div className="space-y-3">
          {data.items.map((it, i) => (
            <div key={i} className="rounded-lg border p-3">
              <p className="text-sm font-medium">〔{i + 1}〕{it.title}</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 mt-1">
                <p className="text-sm"><span className="font-medium">目的：</span>{it.objective}</p>
                <p className="text-sm"><span className="font-medium">担当：</span>{it.owner}</p>
                <p className="text-sm"><span className="font-medium">リズム：</span>{it.cadence}</p>
                <p className="text-sm"><span className="font-medium">指標：</span>{it.metric}</p>
                <p className="text-sm"><span className="font-medium">期限：</span>{it.deadline}</p>
              </div>
            </div>
          ))}
        </div>
