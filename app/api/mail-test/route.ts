...
function buildLinks(req: NextRequest, rid?: string | null) {
  ...
  const reportLink = rid ? `${base}/report/${encodeURIComponent(rid)}` : `${base}/report`;
  const consultLink = rid ? `${bookingBase}?rid=${encodeURIComponent(rid)}` : bookingBase;
...
export async function GET(req: NextRequest) {
  ...
  const name        = param(q, 'name', 'toName', 'userName', 'recipientName') || 'SACHIKOさん';
