...
function buildLinks(
  req: NextRequest,
  rid?: string | null,
  overrides?: { report?: string; consult?: string },
) {
  ...
  const reportBase =
    overrides?.report ||
    (rid ? `${base}/report/${encodeURIComponent(rid)}` : `${base}/report`);
  const consultBase =
    overrides?.consult ||
    (rid ? `${bookingBase}?rid=${encodeURIComponent(rid)}` : bookingBase);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const typeName =
      param(body, "samuraiType", "typeName", "type") || "（タイプ判定中）";
    const companySize =
      param(body, "company_size", "companySize", "size") || "";
    ...
    const input: ReportEmailV2Input = {
      ...
      reportLink,
      consultLink,
      titlePrefix: is51Plus(companySize)
        ? "【武将タイプ診断アプリ特典】"
        : "【武将タイプ診断】",
    };
