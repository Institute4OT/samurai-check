...
export type SamuraiSlug =
  | 'sanada' | 'oda' | 'toyotomi' | 'tokugawa' | 'uesugi' | 'saito' | 'imagawa';
...
/** 苗字スラッグ（正） → 日本語 */
export const SLUG_TO_JA: Record<SamuraiSlug, SamuraiJa> =
  Object.entries(JA_TO_SLUG).reduce((acc, [ja, slug]) => {
...
/** （必要なら）旧キーで返す互換マップ */
export const JA_TO_KEY_LEGACY: Record<SamuraiJa, 'sanada'|'oda'|'hideyoshi'|'ieyasu'|'uesugi'|'saito'|'imagawa'> = {
  '真田幸村型': 'sanada',
  '織田信長型': 'oda',
  '豊臣秀吉型': 'hideyoshi',
  '徳川家康型': 'ieyasu',
