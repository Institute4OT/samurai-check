  ...
  for (const a of answers) {
    ...
    if (a.id === 2) {
      ...
      if (
        ...
        flags.q2TopDownAll = true;
      if (hit(t, "最終的な判断は自分") && hit(t, "一部は任せる"))
        flags.q2PartialDelegate = true;
      if (hit(t, "担当者を決め") || hit(t, "小さく試して"))
        flags.q2ExperimentAssign = true;
      if (hit(t, "関係部署") && hit(t, "話し合う"))
        flags.q2MeetingDecision = true;
      if (hit(t, "目的と制約") && hit(t, "実行は全て任せる"))
        flags.q2FullDelegation = true;
    }
    ...
    if (a.id === 3) {
      ...
      if (hit(t, "決断が早く") || hit(t, "会議の結果が納得できる"))
        flags.q3DecisionClear = true;
    }

    // Q5 重視すること
    if (a.id === 5) {
      if (hit(t, "やり方") || hit(t, "前例") || hit(t, "勘で決める"))
        flags.q5TraditionHeavy = true;
    ...
    if (a.id === 7) {
      ...
      if (hit(t, "他優先") || hit(t, "着手できていない"))
        flags.q7NoAction = true;
    }
    ...
    if (a.id === 10) {
      ...
      if (hit(t, "必要になったら変わる") || hit(t, "先送り"))
        flags.q10Postpone = true;
    }
    ...
    if (a.id === 16) {
      ...
      if (
        ...
        flags.q16ConsultBossOnly = true;
      if (hit(t, "性別で異なる服装") || hit(t, "制服"))
        flags.q16GenderedUniform = true;
      if (hit(t, "社内の特定者") && hit(t, "受ける"))
        flags.q16InternalSingleWindow = true;
      if (hit(t, "男性が多い") || hit(t, "男性がほとんど"))
        flags.q16MaleDominantMgmt = true;

      if ((hit(t, "複数") && hit(t, "匿名")) || hit(t, "匿名の窓口"))
        flags.q16InternalMultiAnon = true;
      if (
        hit(t, "性別中立") ||
        (hit(t, "服装規定") && hit(t, "中立")) ||
      ...
      if (hit(t, "外部") && (hit(t, "窓口") || hit(t, "複線化")))
        flags.q16ExternalHotline = true;
      if (
        hit(t, "公平性") ||
        (hit(t, "女性管理職") && hit(t, "比率")) ||
        hit(t, "モニタ")
      ) {
