...
export type Flags = {
  ...
  // ★追加：相談窓口そのものが未設置
  noConsultWindow?: boolean;        // 「そういう相談窓口は設置していない／窓口なし」
};
...
export function deriveFlags(answers: AnswerLite[]): Flags {
  ...
  for (const a of answers) {
    ...
    if (a.id === 1) {
      ...
      if (hit(t, '活発に議論できている')) flags.q1HealthyDebate = true;
    }

    // Q2 新しい提案への関わり方
    if (a.id === 2) {
      if (hit(t, '却下する')) flags.q2RejectNew = true;
      if (hit(t, '自分がすべて指示') || hit(t, '自分すべて指示') || hit(t, 'すべて自分が指示')) flags.q2TopDownAll = true;
      if (hit(t, '最終的な判断は自分') && hit(t, '一部は任せる')) flags.q2PartialDelegate = true;
      if (hit(t, '担当者を決め') || hit(t, '小さく試して')) flags.q2ExperimentAssign = true;
    ...
    // Q3 新技術/市場へのスタンス
    if (a.id === 3) {
      if (hit(t, '他社よりも早く') || hit(t, '生み出したい') || hit(t, '積極的に試す') || hit(t, '異業種')) flags.q3Pioneer = true;
      if (hit(t, '成功事例') || hit(t, '安全性を確認') || hit(t, '部分的に関心')) flags.q3Cautious = true;
      if (hit(t, '流行には乗らない') || hit(t, '自社のやり方を貫く')) flags.q3Traditional = true;
    ...
    if (a.id === 8) {
      ...
      if (hit(t, '挑戦しやすい') || hit(t, 'まんべんなく聞こえてくる') || hit(t, '指示が明確')) flags.q8ChallengeFriendly = true;
    }

    // Q11 社員へのスタンス
    if (a.id === 11) {
      if (hit(t, '自分が握っている') || hit(t, '自分が行わないと') || hit(t, '常に確認できる仕組み')) flags.q11Micromanage = true;
      if (hit(t, '信じて任せている')) flags.q11HighTrust = true;
    }

    // Q12 退職傾向
    if (a.id === 12) {
      if (hit(t, '退職後も連絡') || hit(t, 'イベントに顔を出してくれ') || hit(t, '今も連携して一緒に仕事')) flags.q12AlumniPositive = true;
      if (hit(t, '残ってほしい人ほど') || hit(t, '退職代行')) flags.q12AttritionRisk = true;
    }

    // Q13 意思決定の型
    if (a.id === 13) {
      if (hit(t, '根回し') || hit(t, '独断で決める')) flags.q13Backroom = true;
      if (hit(t, 'プロセスに基づいて') || hit(t, '合意が形成') || hit(t, '委ねる場面')) flags.q13RulesBased = true;
    }

    // Q14 バスファクター
    if (a.id === 14) {
      if (hit(t, '自分抜きではどうにもならない') || hit(t, '全部自分')) flags.q14BusFactorHigh = true;
    }
    ...
    if (a.id === 16) {
      ...
      if (
        hit(t, '窓口なし') ||
        ( (hit(t, '相談窓口') || hit(t, '窓口')) && (hit(t, '設置していない') || hit(t, '未設置') || hit(t, '設置なし')) ) ||
        hit(t, 'そういう相談窓口は設置していない')
      ) {
        flags.noConsultWindow = true;
      }
