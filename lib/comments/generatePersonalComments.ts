...
const rules: AnswerRule[] = [
  ...
  {
    q: 0,
    includes: /トップダウン|細かな指示|承認待ち|稟議|決裁が?遅/i,
    emit: [
      {
        key: "delegate_neg",
        category: "challenge",
  ...
  {
    q: 0,
    includes: /1on1|ワンオンワン|面談|対話|フィードバック|傾聴|心理的安全/i,
    emit: [
      {
        key: "comms_pos",
        category: "talent",
  ...
  {
    q: 0,
    includes: /数字|見える化|可視化|記録|計測|メーター|ダッシュボード/i,
    emit: [
      {
        key: "measure_pos",
        category: "talent",
...
export function generatePersonalComments(
  ...
  if (talents.length === 0)
    talents.push(
      "よい流れがあります。今のやり方で『続けること』を決めましょう。",
    );
  if (challenges.length === 0)
    challenges.push(
      "まずは2週間で終わる小さな一歩を1つ決め、終わったら振り返りましょう。",
    );
